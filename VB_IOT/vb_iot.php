...
if (isset($_GET['api'])) {
    ...
    if ($api === 'archive_save') {
        ...
        if ($ok_save === false) {
            $chunk_err = $local_conn->error;
            $local_conn->rollback();
            release_archive_lock($local_conn, $lock_name);
            $local_conn->close();
            $detail = $chunk_err !== '' ? ' Chi tiet SQL: ' . $chunk_err : '';
            json_exit([
                'ok' => false,
                'message' => 'Khong luu duoc du lieu blob theo chunk. Kiem tra kieu cot duLieu va quyen ghi local DB.' . $detail
            ]);
        }
